...
node* kth_node_from_end_recursive(node *head, int k, int &i){
    if(head == NULL)
        return NULL;
    node *n = kth_node_from_end_recursive(head->next, k, i);
    i = i + 1;
 //   cout << "i is " << i << "\n";
    if(i == k){
 //       cout<<"i equals k\n";
...
int main(){
    ...
    */
    ...
    cin >> k;
    int i = 0;
    node *kth_node = kth_node_from_end_recursive(head, k, i);
    cout << endl;
    cout << kth_node->data << endl;
